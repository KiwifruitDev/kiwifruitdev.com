...
(function () {
    ...
    LOAD.setServerInfo = function (serverName, mapName, maxPlayers) {
        ...
        if(mapName.endsWith('_wf'))
            mapName = mapName.replace('_wf', '');
        this.$.mapPreview.src = 'https://image.gametracker.com/images/maps/160x120/garrysmod/' + mapName + '.jpg';

        // If the map is rp_aperture, change the background image in loading-screen
        if(mapName.includes('rp_aperture')) {
            document.getElementsByClassName('loading-screen')[0].style.background = 'url(images/aperture.jpg)';
            document.getElementsByClassName('loading-screen')[0].style.backgroundSize = 'cover';
            document.getElementsByClassName('logo')[0].src = 'images/aperture_logo.png';
        } else {
            document.getElementsByClassName('loading-screen')[0].style.background = 'url(images/blackmesa.jpg)';
            document.getElementsByClassName('loading-screen')[0].style.backgroundSize = 'cover';
            document.getElementsByClassName('logo')[0].src = 'images/blackmesa_logo.png';
        }
    };
